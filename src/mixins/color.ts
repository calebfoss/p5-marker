...
export const color = <T extends typeof Base>(baseClass: T) =>
  class ColorElement extends baseClass {
    static gray(value: number, alpha?: number): string {
      return ColorElement.rgb(value, value, value, alpha);
    }
    static hsb(h: number, s: number, b: number, a?: number) {
      const l = b * (1 - s / 200);
      const sl =
        l === 0 || l === 100 ? 0 : ((b - l) / Math.min(l, 100 - l)) * 100;
      if (typeof a !== "undefined") return this.hsl(h, sl, l, a);
      return ColorElement.hsl(h, sl, l);
    }
    static hsl(h: number, s: number, l: number, a?: number) {
